...
class EventProcessor:
    ...
    async def _generate_and_log_solutions(
        ...
        try:
            ...
            # DEBUG: Log solution structure
            print(f"\n🔍 DEBUG: Solution generated by LLM")
            print(f"   Has immediate_fix: {bool(solution.get('immediate_fix'))}")
            print(f"   Has code_changes: {bool(solution.get('code_changes'))}")
            if solution.get('code_changes'):
                code_changes = solution.get('code_changes')
                if isinstance(code_changes, list):
                    print(f"   Number of code_changes: {len(code_changes)}")
                    for i, change in enumerate(code_changes[:3], 1):  # Show first 3
                        print(f"      Change #{i}:")
                        print(f"         file_path: {change.get('file_path', 'MISSING')}")
                        print(f"         has fixed_code: {bool(change.get('fixed_code'))}")
                        print(f"         has current_code: {bool(change.get('current_code'))}")
                elif isinstance(code_changes, dict):
                    print(f"   code_changes is a dict (should be list!)")
                    print(f"      file_path: {code_changes.get('file_path', 'MISSING')}")
            else:
                print(f"   ⚠️  WARNING: No code_changes in solution!")
