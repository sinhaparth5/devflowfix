...
class GitHubTokenManager:
    ...
    def register_token(
        ...
        """
        Register a GitHub access token.
        ...
        try:
            ...
            token_record = GitHubTokenTable(
                id=f"token_{owner}_{repo or 'org'}_{datetime.now(timezone.utc).timestamp()}",
    ...
    def get_token(self, owner: str, repo: Optional[str] = None) -> Optional[str]:
        """
        Retrieve a GitHub token for a repository.
        
        Tries in order:
        1. Repo-specific token (owner/repo)
        2. Organization token (owner/*)
    ...
    def list_tokens(
        ...
        try:
            ...
            query = db.query(GitHubTokenTable)
    ...
    def validate_token(self, owner: str, repo: Optional[str] = None) -> bool:
        """
        Validate that a token exists and is active.
        
        Makes a test API call to GitHub to ensure token is valid.
        ...
        """
        token = self.get_token(owner, repo)
        if not token:
            return False
